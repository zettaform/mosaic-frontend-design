...
import { sidebarNavigation } from '../config/sidebarNavigation.jsx';
...
function Sidebar({
  ...
  useLayoutEffect(() => {
    ...
    const savedScrollTop = Number(sessionStorage.getItem(SIDEBAR_SCROLL_STORAGE_KEY) || '0');
    sidebar.current.scrollTop = Number.isFinite(savedScrollTop) ? savedScrollTop : 0;
  }, []);
  ...
  useEffect(() => {
    ...
    const persistScrollTop = () => {
      sessionStorage.setItem(SIDEBAR_SCROLL_STORAGE_KEY, String(sidebarElement.scrollTop));
    };

    sidebarElement.addEventListener('scroll', persistScrollTop, { passive: true });

    return () => {
      persistScrollTop();
      sidebarElement.removeEventListener('scroll', persistScrollTop);
    };
  }, []);
  ...
  return (
    <div className="min-w-fit">
      ...
      <div
        id="sidebar"
        className={`relative flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-[100dvh] min-h-0 overflow-hidden w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 shrink-0 border-r border-slate-200/50 bg-[#F9F9FB] dark:border-white/[0.04] dark:bg-[#0F1115] px-3 py-4 sidebar-no-scroll sidebar-preserve-state sidebar-smooth sidebar-transition${
      ...
      >
        {/* Sidebar header — outside scroll region so nav never paints through the logo row */}
        <div className="shrink-0 relative z-10 flex justify-between items-center mb-8 px-2">
          {/* Close button */}
          <button
            ref={trigger}
            className="lg:hidden rounded-lg p-1 text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:rounded-none dark:p-0 dark:text-slate-500 dark:hover:bg-transparent dark:hover:text-slate-400"
          ...
          {/* Logo — signed-in users stay in the app (dashboard), not the marketing home */}
          <Logo className="h-8 w-auto" linkTo={user ? '/dashboard' : '/'} />
        </div>

        <div
          ref={sidebar}
          className="flex-1 min-h-0 overflow-y-scroll lg:overflow-y-auto no-scrollbar sidebar-no-scroll sidebar-preserve-state"
        ...
        >
          ...
          <div className="space-y-8">
            {/* Pages group */}
            <div>
              <h3 className="text-[11px] font-medium text-slate-400/80 dark:text-slate-500/90 uppercase tracking-[0.04em] pl-2">
                <span className="hidden lg:block lg:sidebar-expanded:hidden 2xl:hidden text-center w-6" aria-hidden="true">
                  •••
                </span>
                <span className="lg:hidden lg:sidebar-expanded:block 2xl:block truncate">Pages</span>
              </h3>
              <ul className="mt-3">
            ...
            {/* More group */}
            <div>
              <h3 className="text-[11px] font-medium text-slate-400/80 dark:text-slate-500/90 uppercase tracking-[0.04em] pl-2 mt-6">
                <span className="hidden lg:block lg:sidebar-expanded:hidden 2xl:hidden text-center w-6" aria-hidden="true">
                  •••
                </span>
                <span className="lg:hidden lg:sidebar-expanded:block 2xl:block truncate">More</span>
              </h3>
              <ul className="mt-3">
        ...
        <div className="shrink-0 pt-3 hidden lg:inline-flex 2xl:hidden justify-end">
