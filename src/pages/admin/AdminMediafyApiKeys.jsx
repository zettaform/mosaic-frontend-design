        ...
        >
          <div className="p-6 border-b border-slate-200 dark:border-slate-700">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">
              {isEditing ? 'Update Mediafy API Key' : 'Create Mediafy API Key'}
            </h2>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
              Keys are stored in the <span className="font-mono">mediafyapikeys</span> Azure Table Storage table and used by the Mediafy User Info Azure Function.
            </p>
          </div>
...
function AdminMediafyApiKeys() {
  ...
  const fetchKeys = async (lastKey = null, reset = false, silent = false) => {
    try {
      if (!silent) {
        setLoading(true);
        setError('');
      ...
      const result = await adminApiService.getMediafyApiKeys(pagination.limit, lastKey);
      ...
      if (result.success) {
        ...
        const errorMsg = result.error || 'Failed to fetch Mediafy API keys';
        setError(errorMsg);
        
        // If it's a 404, provide helpful message
        if (errorMsg.includes('404') || errorMsg.includes('Not Found')) {
          setError('Azure Table Storage table "mediafyapikeys" not found. The table will be created automatically when you create your first API key.');
        }
      }
    } catch (err) {
      console.error('Error fetching Mediafy API keys:', err);
      const errorMsg = err.message || 'Error fetching Mediafy API keys';
      setError(errorMsg);
  ...
  const handleCreateKey = async (e) => {
    ...
    try {
      ...
      const result = await adminApiService.createMediafyApiKey(payload);
      if (result.success) {
        ...
        setError(result.error || 'Failed to create Mediafy API key');
      }
    } catch (err) {
      setError(err.message || 'Error creating Mediafy API key');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleUpdateKey = async (e) => {
    ...
    try {
      ...
      const result = await adminApiService.updateMediafyApiKey(editingKey.apiKey, updates);
      if (result.success) {
        setEditingKey(null);
        setFormData({
          apiKey: '',
        ...
        setError(result.error || 'Failed to update Mediafy API key');
      }
    } catch (err) {
      setError(err.message || 'Error updating Mediafy API key');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteKey = async (apiKey) => {
    if (!window.confirm('Delete this Mediafy API key? This cannot be undone.')) return;
    try {
      setError('');
      const result = await adminApiService.deleteMediafyApiKey(apiKey);
      if (result.success) {
        await fetchKeys(null, true);
      } else {
        setError(result.error || 'Failed to delete Mediafy API key');
      }
    } catch (err) {
      setError(err.message || 'Error deleting Mediafy API key');
    }
  };
  ...
  const handleTestKey = async (apiKey) => {
    ...
    try {
      // Must use same API base as adminApiService (Vite :5174 → proxy → server PORT from .env).
      // A hardcoded localhost:3001 breaks when the API runs on another port.
      ...
      let response;
      try {
        response = await fetch(`${API_BASE_URL}/admin/mediafy-api-keys/test?${testParams.toString()}`, {
          method: 'GET',
          headers: {
      ...
      if (!response.ok || responseData.success === false) {
        ...
        setError('');
        await fetchKeys(null, true, true);
      }
    } catch (err) {
  ...
  // Refresh usage counters while this page is open (Azure Function updates table in the background).
  useEffect(() => {
    const tick = () => {
      if (document.visibilityState === 'visible') {
        fetchKeys(null, true, true);
      }
    };
            ...
            >
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-400 mt-1">
                  Manage API keys in <span className="font-mono">mediafyapikeys</span>. Set{' '}
                  <span className="font-mono">AZURE_MEDIAFY_API_KEYS_CONNECTION_STRING</span> in{' '}
                  <span className="font-mono">.env/myapp.env</span> to match your Azure Function storage so usage limits
                  stay in sync with live calls.
                </p>
              </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                <p className="text-lg font-medium text-slate-900 dark:text-slate-100 mb-2">
                  No Mediafy API keys yet
                </p>
                <p className="text-slate-500 dark:text-slate-400 mb-6">
                  Create a key to authenticate requests to the Mediafy user-info endpoint and enforce per-key request limits.
                </p>
                <button
...
export default AdminMediafyApiKeys;
