...
import { RefreshCw, Search, BarChart3, X, ChevronLeft, ChevronRight, Eye, FileText, MessageSquare, Image, Download } from 'lucide-react';
...
const deriveGeneratedTextPreview = (value) => {
  if (!value) return '';
  if (typeof value === 'string') return value;
...
const Campaigns = () => {
  ...
  const fetchCampaignStats = async (overrides = {}) => {
    ...
    try {
      ...
      console.error('[Campaigns] Error fetching campaigns:', fetchError);
      setError(fetchError.message || 'Failed to load campaigns');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openGeneratedTextModal = useCallback((row) => {
    const generatedText = stringifyForDisplay(row.openai_results?.generated_text);
  ...
  const downloadCampaignRecords = useCallback(async (campaignId, campaignName, e) => {
    ...
    try {
      ...
      const formattedData = allRecords.map((row) => ({
        ...
        'Generated Text': stringifyForDisplay(row.openai_results?.generated_text),
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 dark:bg-slate-900">
          <div className="px-4 sm:px-6 lg:px-8 py-6 w-full max-w-7xl mx-auto">
            ...
            <AdvancedDataTable
              ...
              loading={loading}
              density="comfortable"
              emptyMessage="No campaigns recorded yet."
                      ...
                      render: (value, row) => (
                        <div className="w-full max-w-[280px] overflow-hidden">
                          {value ? (
                            ...
                            >
                              {deriveGeneratedTextPreview(value) || '[generated_text JSON]'}
                            </button>
                          ) : (
